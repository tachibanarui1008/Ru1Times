...
|---|---|---|---|---|
| Ru1Daily | `app/reports/YYYY-MM-DD.ts` | `app/report-types.ts` | `app/data.ts` | `/daily` |
...
`scripts/generate-content-index.mjs` discovers these issue files before every build and writes the ignored `app/generated/` index. Do not manually edit the stable adapters. Only `draft: false` issues enter the Ru1Times homepage cards, latest routes, Archive, and static export; `draft: true` files remain local content packages. `/` is the Ru1Times homepage; the header contains only `Home` and `Archive`, while the four channel links live on that homepage.
...
- `/`, the channel route, `/archive`, and the issue archive route render as applicable. Daily archive IDs use `daily-YYYY-MM-DD`; legacy `YYYY-MM-DD` static paths remain compatibility aliases.
