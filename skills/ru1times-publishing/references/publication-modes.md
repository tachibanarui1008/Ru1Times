...
4. Stage only the named issue file and other files the author explicitly approved for that issue. Do not edit or stage channel data adapters for a normal issue; the index is generated at build time. Never stage `app/generated/`, `out/`, unrelated edits, or all files at once.
...
2. Finalize publication status with `draft: false`. Confirm timestamps, edition number, sources, and author-provided `ai_credit`.
...
4. Review the final diff. For a normal issue, stage only its typed `.ts` content package and explicitly required related changes; never stage the ignored generated index or use broad staging commands. Do not include unrelated README, design, finance, export, or workspace changes.
...
The site is source-driven, not hand-authored-HTML-driven. A user authors an issue by creating exactly one correct typed issue file. `pnpm run content:index` validates it and automatically supplies the channel arrays; no manual registry edit is required. Only `draft: false` issues enter the public build. Pushing the approved content file to `main` lets GitHub Actions generate and deploy HTML. A standalone HTML file is not automatically added to the latest route or Archive.
