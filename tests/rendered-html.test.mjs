...
test("server-renders the Ru1Times homepage", async () => {
  ...
  assert.match(html, /行政命令的边界：出生公民权争议再次回到法院/);
...
test("server-renders the Ru1Finance edition", async () => {
  ...
  assert.equal(response.status, 200);
  const html = await response.text();
  assert.match(html, /Stock dividends: cash income, ex-dividend mechanics and total return/);
  assert.match(html, /publication-masthead/);
  ...
  assert.match(html, /股票分红：收到现金以后，你真的变得更富了吗/);
  assert.match(html, /今日总览/);
  assert.match(html, /3,952\.18/);
  assert.match(html, /66,405\.56/);
  assert.match(html, /6,788\.79/);
