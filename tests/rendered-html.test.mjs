...
test("server-renders the Ru1Finance edition", async () => {
  ...
  assert.equal(response.status, 200);
  const html = await response.text();
  assert.match(html, /Shadow banking: when credit behaves like a bank outside the bank/);
  assert.match(html, /publication-masthead/);
  assert.match(html, /第 003 期/);
  ...
  assert.match(html, /影子银行：谁在替银行做“借短投长”/);
  assert.match(html, /今日总览/);
  assert.match(html, /3,882\.01/);
  assert.match(html, /65,528\.09/);
  assert.match(html, /6,696\.96/);
  assert.match(html, /53,277\.01/);
  assert.match(html, /26,136\.56/);
  assert.match(html, /quote-change (up|down)/);
  assert.match(html, /2026-08-24 收盘/);
  assert.match(html, /中植集团等 316 家企业/);
